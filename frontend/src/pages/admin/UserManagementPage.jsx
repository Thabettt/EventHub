...
const UserManagementPage = () => {
  ...
  // Pagination Logic
  const filteredUsers = users; // Server now filters and paginates
  const indexOfFirstUser = (currentPage - 1) * usersPerPage;
  const indexOfLastUser = Math.min(indexOfFirstUser + usersPerPage, totalUsers);
  const currentUsers = users; // Server sends sliced data
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 dark:from-gray-900 dark:via-gray-900 dark:to-gray-800 transition-all duration-300 relative">
            ...
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400 font-medium mt-0.5">
                  {totalUsers} platform users • {currentUser?.name || "Admin"}
                </p>
              </div>
            </div>
          </div>
      ...
      <div
        className={`flex-1 ${deviceInfo.isDesktop ? "p-8" : "p-4"} overflow-y-auto`}
      >
        ...
        {isLoading ? (
          <LoadingSpinner variant="page" size="xl" message="Loading Users" />
        ) : error ? (
          ...
          <>
            ...
            {deviceInfo.isDesktop ? (
                  ...
                  <table className="min-w-full">
                    <thead className="bg-gray-50/70 dark:bg-gray-900/70">
                      <tr>
                        ...
                        <th className="px-8 py-6 text-left text-sm font-black text-gray-900 dark:text-white uppercase tracking-wider text-right">
                          Actions
                        </th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-200/50 dark:divide-gray-700/50">
                      {currentUsers.map((user) => (
                        <tr
                          key={user._id}
                          className="hover:bg-gray-50/50 dark:hover:bg-gray-900/50 transition-colors duration-200 group"
                        >
              ...
              <div className="space-y-4">
                {currentUsers.map((user) => (
                  <div
                    key={user._id}
                    className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl rounded-2xl p-4 shadow-lg border border-gray-200/50 dark:border-gray-700/50"
                  >
                    <div className="flex items-start justify-between mb-4">
                      ...
                      <div
                        className={`px-2 py-1 rounded-lg text-xs font-bold ${getRoleBadgeStyle(user.role).split(" ")[0]} ${getRoleBadgeStyle(user.role).split(" ")[1]} ${getRoleBadgeStyle(user.role).split(" ")[2] || ""}`}
                      >
                        {user.role}
                      </div>
                    </div>
            ...
            {totalPages > 1 && (
              <div
                className={`mt-6 flex items-center justify-between ${deviceInfo.isMobile ? "flex-col gap-4" : ""}`}
              >
                <div className="text-sm text-gray-500 dark:text-gray-400 font-medium">
                  ...
                  of <span className="font-bold">{totalUsers}</span>
                </div>
                <div className="flex items-center space-x-2">
                  <button
