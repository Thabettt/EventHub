        ...
        <div className="bg-gradient-to-r from-indigo-900/50 to-purple-900/50 rounded-xl p-6 border border-white/10 text-center">
          ...
          <div className="flex justify-center gap-4 text-sm text-gray-300">
            <span className="flex items-center gap-1">
              <Icons.Calendar /> {new Date(booking.date).toLocaleDateString()}
            </span>
            <span className="flex items-center gap-1">
...
const BookingStatus = ({ status }) => {
  ...
  return (
    <span
      className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider border ${currentStyle} shadow-[0_0_10px_rgba(0,0,0,0.1)]`}
    >
      {status}
    </span>
  );
  ...
  >
    ...
    <div className="sm:w-32 bg-gradient-to-br from-indigo-600 via-purple-600 to-slate-800 dark:from-indigo-900 dark:via-purple-900 dark:to-slate-900 flex flex-col items-center justify-center p-4 relative overflow-hidden text-white">
      ...
      <div className="relative z-10 text-center">
        ...
        <div className="text-xs font-bold text-white/75 uppercase tracking-widest">
          Ticket
        </div>
      </div>
    ...
    <div className="flex-1 p-5 sm:p-6 flex flex-col justify-between relative bg-white/80 dark:bg-white/5 backdrop-blur-sm">
      ...
      <div className="flex justify-between items-end border-t border-black/5 dark:border-white/10 pt-4 mt-2">
        <div>
          <p className="text-xs text-gray-500 dark:text-gray-500 uppercase font-bold tracking-wider mb-0.5">
            Quantity
          </p>
...
const ProfilePage = () => {
  ...
  const showMessage = (type, msg, duration = 5000) => {
    ...
    if (duration)
      setTimeout(() => {
        type === "success" ? setSuccessMessage("") : setError("");
      }, duration);
  };
  ...
  const getRoleBadge = () => {
    ...
    const styles = {
      admin:
        "bg-red-500/20 text-red-600 dark:text-red-300 border-red-500/40 shadow-red-500/20",
      organizer:
        "bg-purple-500/20 text-purple-600 dark:text-purple-300 border-purple-500/40 shadow-purple-500/20",
    ...
    const roleKey = currentUser.isAdmin
      ? "admin"
      : currentUser.isOrganizer
        ? "organizer"
        : "user";
    const displayRole = currentUser.isAdmin
      ? "System Admin"
      : currentUser.isOrganizer
        ? "Organizer"
        : "Attendee";
    ...
    return (
      <span
        className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-widest border shadow-lg backdrop-blur-md ${styles[roleKey]}`}
      >
        {displayRole}
      </span>
    );
  ...
  const visibleBookings = viewAllTickets
    ? userBookings
    : userBookings.slice(0, 4);
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-8">
              <div className="flex items-center justify-between mb-8">
                ...
                <Link
                  to="/events"
                  className="text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 dark:hover:text-indigo-300 text-sm font-bold flex items-center gap-1"
                >
                  Browse Events{" "}
                  <svg
                    className="w-4 h-4"
              ...
              {isLoading ? (
                ...
                >
                  <div className="w-16 h-16 bg-gray-100 dark:bg-white/5 rounded-full flex items-center justify-center mx-auto mb-4 text-3xl">
                    🎫
                  </div>
                  ...
                  <Link
                    to="/events"
                    className="inline-block px-6 py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl font-bold shadow-lg shadow-indigo-900/30 transition-all hover:scale-105"
                  >
                    Discover Events
                  </Link>
                </motion.div>
          ...
          <form onSubmit={handleProfileUpdate} className="space-y-6">
            ...
            <div className="flex gap-4 pt-4">
              <button
                ...
                className="flex-1 py-3 rounded-xl bg-white/5 hover:bg-white/10 text-gray-400 font-bold transition-colors"
              >
                Cancel
              </button>
              <button
                type="submit"
                disabled={isLoading}
                className="flex-1 py-3 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold shadow-lg shadow-indigo-900/20 transition-all"
              >
